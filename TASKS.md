...
| SLACK-001 | DONE | CHAN-001, HARNESS-001 | Add Slack Socket Mode adapter | `spec.interfaces.slack.name` renders one app per agent. DM sessions, mention-started public/private channel threads, registered follow-ups, acknowledgements, deduplication, bounded plain-text replies, and secret-safe logging are implemented. Offline validation passes in the current 160-test suite. On 2026-08-04 the operator confirmed live DM and channel-thread conversations work as expected through Harness. |
...
| PKG-001 | DONE | GATEWAY-001 | Replace zip Lambda packaging with container images | `containers/manifest.json` is the single source of truth for buildable images; `scripts/containers.py` provides source-digest change detection and immutable digest resolution. Both repository Lambdas are deployed as digest-pinned `arm64` images. The GitHub broker migration and live repository-list proof passed. The `slack-oauth-callback` image `src-1ddf55fe0e03` resolves to `sha256:80d4fc9962a491300005855a8d7a5ad5467a9f09c1129f3edb5d72e1119027e7`; its 2026-08-04 apply added 10 resources, changed 0, destroyed 0, and post-apply verification found `Active`, `LastUpdateStatus: Successful`, the scoped API Gateway invoke permission, and no Terraform drift. Replacement-triggered permissions protect both Lambda replacement paths. No zip-packaged Lambda remains in source or deployment. |
