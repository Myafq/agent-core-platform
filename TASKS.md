...
| GATEWAY-001 | IN_PROGRESS | ARCH-003, GHAPP-002 | Implement IAM Gateway and Lambda target | Operator applied the reviewed platform plan on 2026-07-25. Read-only checks found the Gateway ARN/ID and Lambda ARN outputs; Lambda `github-app-tool` is `Active`, last update `Successful`, Python 3.11, with its scoped role. The Gateway Lambda event adapter was then corrected to use AgentCore client-context tool metadata; package/review/apply that correction before any invocation. |
| HARNESS-002 | IN_PROGRESS | HARNESS-001, GATEWAY-001 | Attach exact GitHub tools to Harness | Harness v5 is `READY`; source permits only `@github-read/listRepositories`, `@github-read/getRepository`, and `@github-read/getFile`. Source now selects `us.amazon.nova-2-lite-v1:0` through `converse_stream`; its role permits streaming only through that US inference profile and its three documented backing foundation models. Pending reviewed apply and one restricted retry. |
