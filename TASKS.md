...
| SLACK-004 | IN_PROGRESS | SLACK-002 | Replace the localhost OAuth callback with a production Lambda | Added `services/slack_oauth_callback` (Lambda behind `modules/slack-oauth-callback` HTTP API Gateway), the only public route being `GET /slack/oauth/callback`; `contracts/slack_oauth_state.py` signs/verifies expiring per-agent install state; `clients/slack/reconciliation.py` gained a generated `state_signing_key`, migration for pre-existing apps, exact-`redirect_uri` OAuth exchange, and a no-write `install-url` command; `render_slack_manifest.py`/`reconcile.py`/`launcher.py` now require an explicit `--redirect-uri` and the hardcoded `https://localhost/slack/oauth/callback` is fully removed from source and docs. All 126 repository unit tests pass, including 27 new tests for the callback and 20 for state signing plus updated reconciliation/manifest/launcher tests, covering success, missing/malformed/expired/tampered state, workspace/app/bot-token mismatches, exact `redirect_uri` equality, legacy credential migration, SSM paths and `SecureString` writes, and idempotent-retry/no-corruption. `terraform validate` passed and an authorized read-only `terragrunt plan` for `platform/slack-oauth-callback` showed exactly 10 resources to add, 0 to change, 0 to destroy. No apply, Slack manifest update, installation, or live callback invocation has run. |
