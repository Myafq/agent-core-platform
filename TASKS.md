...
| HARNESS-003 | IN_PROGRESS | HARNESS-002 | Add the native coding workspace | The immutable ARM64 coding image `github-app-tool-coding@sha256:455de191c7d1339fb4124c8570cd71b3b4bda14223c2ecd8bbce41dc2c658d3b` is deployed on Harness v14. The superseded, unapplied VPC/NAT/EFS workspace unit and mock dependency were removed on 2026-08-03 to avoid idle home-lab cost. Harness source now attaches AgentCore-managed session storage at `/mnt/workspace` in public mode; it persists only for the same `runtimeSessionId`. Init/validate and its reviewed plan passed: one controlled environment update, no AWS resources created, changed, or destroyed. Apply remains pending; then prove clone, edit, test, commit, push, and PR in one Harness session. |
