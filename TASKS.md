...
| HARNESS-003 | IN_PROGRESS | HARNESS-002 | Add the native coding workspace | The immutable ARM64 coding image `github-app-tool-coding@sha256:455de191c7d1339fb4124c8570cd71b3b4bda14223c2ecd8bbce41dc2c658d3b` was superseded by the credential-helper image `github-app-tool-coding@sha256:54450f0aeb93ae43d92f184802fbe12c271e7254eb5c73ae438b62a734b11686`. `terragrunt run --all -- plan` passed and was applied. Read-only verification found broker Lambda `Active`/`Successful`; Harness v17 `READY`, the exact image digest, non-secret broker function name, and one scoped `lambda:InvokeFunction` permission. The live agent found its shell lacks a configured AWS region; source now injects the non-secret deployment region and helper explicitly selects it, but this needs image/Harness deployment. The helper then reached the broker, which safely classified token minting as `github_auth_failed`; verify the installed App has approved Contents, Pull requests, and Issues write before retrying. No live token mint, clone, commit, push, or PR proof has run. The Harness dependency shallow-merges real state with non-secret broker-output mocks for `validate`/`plan`; apply requires real dependency state. The superseded, unapplied VPC/NAT/EFS workspace unit and mock dependency were removed on 2026-08-03 to avoid idle home-lab cost. AgentCore-managed session storage is applied at `/mnt/workspace` in public mode and persists only for the same `runtimeSessionId`. Temporary risk-accepted mode: the Harness may invoke only the existing broker Lambda, which mints a new selected-repository App token for `git`/`gh`; the private key remains Lambda-only. Tokens are not Terraform inputs, Harness environment values, or persisted files. They are reachable by the root shell and may be exfiltrated or used for any operation allowed by their one-hour repository/permission scope. |
...
| WRITE-001 | IN_PROGRESS | HARNESS-003 | Add autonomous GitHub mutations | Temporary risk-accepted mode: native `git`/`gh` uses a fresh selected-repository GitHub App token from the existing broker Lambda. No confirmation turn. The private key remains Lambda-only, but the root-capable Harness can retrieve and exfiltrate each one-hour token; do not expose it through logs, prompts, files, or Terraform. Replace this with a credential-isolated MCP/service design before production use. Do not retire the Gateway slice before native live proof. |
