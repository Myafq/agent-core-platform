...
| HARNESS-003 | IN_PROGRESS | HARNESS-002 | Add the native coding workspace | Created private ECR repository `github-app-tool-coding` with scan-on-push and 10-image retention. Build/push the ARM64 image with Git and GitHub CLI; enable built-in shell/file tools; create EFS/VPC/NAT resources; mount EFS at `/mnt/workspace`; then prove clone, edit, test, commit, push, and PR in one Harness session. |
