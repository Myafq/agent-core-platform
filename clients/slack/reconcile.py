...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("command", choices=("plan", "apply", "complete-oauth", "set-app-token"))
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "plan":
            ...
            print(json.dumps(reconciler.plan(spec, args.workspace_id, paths, adopt_app_id=args.adopt_app_id).safe_output(), sort_keys=True))
            return 0
        reconciler = SlackReconciler(_ssm(args), SlackSdkProvisionerApi())
        if args.command == "apply":
            print(json.dumps(reconciler.apply(spec, args.workspace_id, paths, adopt_app_id=args.adopt_app_id).safe_output(), sort_keys=True))
            return 0
        ...
        if args.command == "complete-oauth":
            ...
            binding = reconciler.complete_oauth(args.workspace_id, paths, code)
